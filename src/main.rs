...
impl GameState {
    ...
    pub fn apply(&mut self, command: Command) {
        ...
        match command {
            ...
            Ladder { starts: s, ends: e } => self.board.set_cell_prop(s, LadderStart { end: e }),
            Snake { starts: s, ends: e } => self.board.set_cell_prop(s, SnakeStart { end: e }),
...
impl Command {
    ...
    fn readFrom(line: String) -> Command {
        ...
        match (keyword, parameters.collect::<Vec<_>>()) {
            ...
            ("players", ref ps) if ps.len() == 1 => Players(num8(ps[0]) as PlayerIx),
...
mod test {
    ...
    fn parse_sample_input() {
        ...
        assert!(Command::readFrom(lines[1].into()) == Players(2));
        assert!(Command::readFrom(lines[2].into()) == Dice(vec![1, 2, 2, 2, 2]));
        ...
        assert!(Command::readFrom(lines[8].into()) == Turns(10));
